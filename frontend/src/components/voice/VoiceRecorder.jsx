...
export default function VoiceRecorder({ recipientId, classId, onSent }) {
  ...
  async function stopAndSend() {
    ...
    try {
      ...
      const note = await api.post('/voice-notes', { storage_path: path, recipient_id: recipientId, class_id: classId, duration_secs: duration })
      // Trigger transcription in background
      api.post(`/voice-notes/${note.id}/transcribe`).catch(() => {})
      onSent?.(note)
      setState('idle')
    } catch (e) {
      setError(e.message)
